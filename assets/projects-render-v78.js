(function () {
  ...
  function projectHasContent(item) {
    return [item && item.titulo_pt, item && item.titulo_en, item && item.titulo]
      .some((value) => String(value || "").trim());
  }

  function projectIsVisible(item) {
    if (item && item.visibilidade_definida === true) return item.visivel !== false;
    // Compatibilidade com itens preenchidos por versões antigas do painel,
    // que eram gravados com visivel=false por padrão.
    return projectHasContent(item);
  }

  function visibleProjects(items, lang) {
    return (items || [])
      .filter((item) => item && projectIsVisible(item))
