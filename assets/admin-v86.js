(function () {
  ...
  const collections = {
    ...
    "livros-didaticos": {
      ...
      fields: [
        ...
        ["botao1_label", "text", "Link 1 — texto"],
        ["botao1_url", "text", "Link 1 — URL"],
        ["botao1_imagem", "image-upload", "Link 1 — thumbnail"],
        ["botao2_label", "text", "Link 2 — texto"],
        ["botao2_url", "text", "Link 2 — URL"],
        ["botao2_imagem", "image-upload", "Link 2 — thumbnail"],
        ["botao3_label", "text", "Link 3 — texto"],
        ["botao3_url", "text", "Link 3 — URL"],
        ["botao3_imagem", "image-upload", "Link 3 — thumbnail"],
        ["botao4_label", "text", "Link 4 — texto"],
        ["botao4_url", "text", "Link 4 — URL"],
        ["botao4_imagem", "image-upload", "Link 4 — thumbnail"],
        ["botao5_label", "text", "Link 5 — texto"],
        ["botao5_url", "text", "Link 5 — URL"],
        ["botao5_imagem", "image-upload", "Link 5 — thumbnail"],
        ["botao6_label", "text", "Link 6 — texto"],
        ["botao6_url", "text", "Link 6 — URL"],
        ["botao6_imagem", "image-upload", "Link 6 — thumbnail"],
  ...
  function projectHasContent(item) {
    return [
      item && item.titulo_pt,
      item && item.titulo_en,
      item && item.titulo,
      item && item.descricao_pt,
      item && item.descricao_en,
      item && item.descricao
    ].some((value) => String(value || "").trim());
  }

  function projectVisibility(item) {
    if (item && item.visibilidade_definida === true) return item.visivel !== false;
    // Migração: versões anteriores criavam seções preenchidas com visivel=false.
    // Projetos com conteúdo passam a ser mostrados; seções vazias continuam ocultas.
    return projectHasContent(item) ? true : item && item.visivel !== false;
  }
  ...
  function normalizeProjectItem(item) {
    ...
    if (hasBilingualFields) {
      return {
        ...
        visivel: projectVisibility(item),
        visibilidade_definida: item.visibilidade_definida === true,
    ...
    pairs.forEach(({ pt, en, index }) => {
      ...
      ready.push({
        ...
        item: {
          ...
          visibilidade_definida: Boolean(ptItem.visibilidade_definida || enItem.visibilidade_definida),
  ...
  async function uploadImageToGitHub(file, folder = "livros-academicos") {
    ...
    setStatus(folder === "sobre" ? "Enviando a foto para o GitHub..." : "Enviando a capa para o GitHub...");

    await githubRequest(`https://api.github.com/repos/${OWNER}/${REPO}/contents/${repoPath}`, {
      method: "PUT",
      body: JSON.stringify({
        message: `${folder === "sobre" ? "Atualiza foto" : "Adiciona capa"}: ${fileName}`,
        content,
        branch: BRANCH
      })
    });
  ...
  // V85 — Livros didáticos usam apenas thumbnails individuais dos links.
  // O campo legado `imagem` pode continuar no JSON para compatibilidade, mas nunca aparece no painel.
  if (collections["livros-didaticos"] && Array.isArray(collections["livros-didaticos"].fields)) {
    collections["livros-didaticos"].fields = collections["livros-didaticos"].fields.filter(([name]) => name !== "imagem");
    if (collections["livros-didaticos"].blank) delete collections["livros-didaticos"].blank.imagem;
  }
  ...
  function renderEditor() {
    ...
    if (currentCollection === "grupos-pesquisa") {
      const p = document.createElement("p");
      p.className = "hint";
      p.textContent = "Cadastre cada grupo uma única vez. Preencha o nome e a descrição em português e em inglês; o hiperlink, a visibilidade e a ordem são compartilhados pelas duas versões.";
      form.appendChild(p);
    }

    if (currentCollection === "livros-didaticos") {
      const p = document.createElement("p");
      p.className = "hint";
      p.textContent = "Use apenas os blocos Link 1 a Link 6. Cada link tem texto, URL e thumbnail próprios. O antigo hiperlink externo não é mais exibido no site.";
      form.appendChild(p);
    }
  ...
  function normalizeResearchGroup(values, existing = {}) {
    const id = String(existing.id || values.id || `grupo-${Date.now()}`).trim();
    return {
      ...existing,
      ...values,
      id,
      nome_pt: String(values.nome_pt ?? existing.nome_pt ?? "").trim(),
      nome_en: String(values.nome_en ?? existing.nome_en ?? "").trim(),
      descricao_pt: String(values.descricao_pt ?? existing.descricao_pt ?? "").trim(),
      descricao_en: String(values.descricao_en ?? existing.descricao_en ?? "").trim(),
      link: String(values.link ?? existing.link ?? "").trim(),
      visivel: values.visivel !== false,
      ordem: values.ordem === "" ? 999 : Number(values.ordem ?? existing.ordem ?? 999)
    };
  }

  function researchGroupMatchIndex(items, group) {
    if (!Array.isArray(items)) return -1;
    const byId = items.findIndex(item => String(item && item.id || "") === String(group.id || ""));
    if (byId >= 0) return byId;

    const targetPt = String(group.nome_pt || "").trim().toLowerCase();
    const targetEn = String(group.nome_en || "").trim().toLowerCase();
    const targetLink = String(group.link || "").trim().toLowerCase();
    return items.findIndex(item => {
      const samePt = targetPt && String(item && item.nome_pt || "").trim().toLowerCase() === targetPt;
      const sameEn = targetEn && String(item && item.nome_en || "").trim().toLowerCase() === targetEn;
      const sameLink = targetLink && String(item && item.link || "").trim().toLowerCase() === targetLink;
      return (samePt && sameEn) || (samePt && sameLink) || (sameEn && sameLink);
    });
  }

  async function saveResearchGroupDirectly() {
    const config = collections["grupos-pesquisa"];
    const localItems = getAllItems(config);
    if (currentActualIndex === null || !localItems[currentActualIndex]) {
      throw new Error("Nenhum grupo selecionado. Clique em novo item, preencha os campos e publique.");
    }

    const values = collectEditorValues();
    const group = normalizeResearchGroup(values, localItems[currentActualIndex]);
    if (!group.nome_pt && !group.nome_en) {
      throw new Error("Preencha o nome do grupo em português ou em inglês antes de publicar.");
    }

    // Mantém o mesmo ID na memória para impedir duplicação em uma segunda tentativa.
    localItems[currentActualIndex] = group;
    setStatus("Lendo a versão mais recente dos grupos no GitHub...");

    const latestFile = await githubRequest(apiURL(config.path));
    const latestData = JSON.parse(decodeBase64Unicode(latestFile.content));
    const remoteItems = Array.isArray(latestData.items) ? [...latestData.items] : [];
    const remoteIndex = researchGroupMatchIndex(remoteItems, group);

    if (remoteIndex >= 0) remoteItems[remoteIndex] = { ...remoteItems[remoteIndex], ...group };
    else remoteItems.push(group);

    const nextData = { ...latestData, items: remoteItems };
    setStatus("Publicando o grupo de pesquisa no GitHub...");

    const result = await githubRequest(`https://api.github.com/repos/${OWNER}/${REPO}/contents/${config.path}`, {
      method: "PUT",
      body: JSON.stringify({
        message: `Atualiza grupo de pesquisa: ${group.nome_pt || group.nome_en}`,
        content: encodeBase64Unicode(JSON.stringify(nextData, null, 2) + "\n"),
        sha: latestFile.sha,
        branch: BRANCH
      })
    });

    const savedSha = result && result.content && result.content.sha ? result.content.sha : "";
    if (!savedSha) throw new Error("O GitHub não confirmou o arquivo atualizado.");

    // Confirma diretamente pela API, sem depender do cache do site ou da Vercel.
    const confirmedFile = await githubRequest(apiURL(config.path));
    const confirmedData = JSON.parse(decodeBase64Unicode(confirmedFile.content));
    const confirmedItems = Array.isArray(confirmedData.items) ? confirmedData.items : [];
    const confirmedIndex = researchGroupMatchIndex(confirmedItems, group);
    const confirmed = confirmedIndex >= 0 ? confirmedItems[confirmedIndex] : null;

    const fieldsMatch = confirmed &&
      String(confirmed.nome_pt || "") === group.nome_pt &&
      String(confirmed.nome_en || "") === group.nome_en &&
      String(confirmed.descricao_pt || "") === group.descricao_pt &&
      String(confirmed.descricao_en || "") === group.descricao_en &&
      String(confirmed.link || "") === group.link;

    if (!fieldsMatch) {
      throw new Error("O GitHub recebeu a solicitação, mas os campos do grupo não foram confirmados no arquivo.");
    }

    currentData = confirmedData;
    currentSha = confirmedFile.sha;
    currentActualIndex = confirmedIndex;
    renderList();
    renderEditor();
    setStatus("Grupo de pesquisa salvo e confirmado no GitHub. A Vercel deve atualizar o site em instantes.");
  }

  async function deleteResearchGroupDirectly() {
    const config = collections["grupos-pesquisa"];
    const localItems = getAllItems(config);
    if (currentActualIndex === null || !localItems[currentActualIndex]) return;
    const group = normalizeResearchGroup({}, localItems[currentActualIndex]);
    if (!confirm("Tem certeza que deseja excluir este grupo de pesquisa?")) return;
    ...
    const remoteItems = Array.isArray(latestData.items) ? [...latestData.items] : [];
    const remoteIndex = researchGroupMatchIndex(remoteItems, group);
    if (remoteIndex < 0) throw new Error("O grupo não foi localizado no arquivo do GitHub.");
    remoteItems.splice(remoteIndex, 1);

    const nextData = { ...latestData, items: remoteItems };
    await githubRequest(`https://api.github.com/repos/${OWNER}/${REPO}/contents/${config.path}`, {
      method: "PUT",
      body: JSON.stringify({
        message: `Remove grupo de pesquisa: ${group.nome_pt || group.nome_en}`,
        content: encodeBase64Unicode(JSON.stringify(nextData, null, 2) + "\n"),
        sha: latestFile.sha,
        branch: BRANCH
      })
    ...
    renderList();
    renderEditor();
    setStatus("Grupo de pesquisa removido do GitHub.");
  }
  ...
  async function saveCurrent() {
    ...
    if (currentCollection === "grupos-pesquisa") {
      await saveResearchGroupDirectly();
      return;
    }
    ...
    if (config.mode === "page-singleton") {
      ...
      // V86 — a página Sobre é um page-singleton. O upload precisa acontecer
      // antes da montagem e publicação do registro; caso contrário, o arquivo
      // selecionado fica apenas na pré-visualização e a foto continua vazia.
      ...
      const savedPhoto = currentCollection !== "page-sobre" ||
        (confirmedPage && String(confirmedPage.foto ?? "") === String(mergedPage.foto ?? ""));

      if (!savedPt || !savedEn || !savedPhoto) {
        throw new Error(currentCollection === "page-sobre"
          ? "O GitHub respondeu, mas a nova foto não foi confirmada. Recarregue o painel e tente novamente."
          : "O GitHub respondeu, mas a descrição não foi confirmada. Recarregue o painel e tente novamente.");
      }

      setStatus(currentCollection === "page-sobre"
        ? "Página Sobre salva. A foto e os textos foram confirmados no GitHub."
        : `Descrição de ${config.label} salva em português e inglês.`);
      return;
    }
    ...
    if (currentCollection === "livros-academicos" || currentCollection === "livros-didaticos") {
      const uploadFolder = currentCollection === "livros-didaticos" ? "livros-didaticos" : "livros-academicos";
      const imageFields = collections[currentCollection].fields
        .filter(([, type]) => type === "image-upload")
        .map(([name]) => name);

      for (const fieldName of imageFields) {
        const fileInput = $("editorForm").elements[`${fieldName}_file`];
        const selectedFile = fileInput && fileInput.files ? fileInput.files[0] : null;
        if (selectedFile) {
          values[fieldName] = await uploadImageToGitHub(selectedFile, uploadFolder);
        }
      }
    }
    ...
    if (currentCollection === "projetos") {
      ...
      mergedValues.visivel = values.visivel !== false;
      mergedValues.visibilidade_definida = true;
      delete mergedValues.idioma;
    }

    if (currentCollection === "grupos-pesquisa") {
      mergedValues.id = String(mergedValues.id || `grupo-${Date.now()}`);
      mergedValues.nome_pt = String(values.nome_pt ?? mergedValues.nome_pt ?? "").trim();
      mergedValues.nome_en = String(values.nome_en ?? mergedValues.nome_en ?? "").trim();
      mergedValues.descricao_pt = String(values.descricao_pt ?? mergedValues.descricao_pt ?? "").trim();
      mergedValues.descricao_en = String(values.descricao_en ?? mergedValues.descricao_en ?? "").trim();
      mergedValues.link = String(values.link ?? mergedValues.link ?? "").trim();
      mergedValues.visivel = values.visivel !== false;
      mergedValues.ordem = values.ordem === "" ? 999 : Number(values.ordem ?? mergedValues.ordem ?? 999);

      if (!mergedValues.nome_pt && !mergedValues.nome_en) {
        throw new Error("Preencha o nome do grupo em português ou em inglês antes de publicar.");
      }
    }
    ...
    if (currentCollection === "grupos-pesquisa") {
      const savedItems = Array.isArray(currentData && currentData.items) ? currentData.items : [];
      const confirmed = savedItems.find((item) => String(item.id || "") === mergedValues.id) ||
        savedItems.find((item) => String(item.nome_pt || "") === mergedValues.nome_pt && String(item.nome_en || "") === mergedValues.nome_en);
      if (!confirmed) {
        throw new Error("O GitHub respondeu, mas o grupo não foi confirmado no arquivo. Recarregue a categoria e tente novamente.");
      }
      setStatus("Grupo de pesquisa publicado e confirmado no GitHub. O site será atualizado pela Vercel em instantes.");
    }
  }
  ...
  async function deleteItem() {
    ...
    if (currentCollection === "grupos-pesquisa") {
      await deleteResearchGroupDirectly();
      return;
    }
