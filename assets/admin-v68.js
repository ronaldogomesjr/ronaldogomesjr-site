(function () {
  ...
  function publicationPageCollection(pageId, label, defaults) {
    return {
      ...
      label,
      labelField: "title_pt",
      meta: item => `${item.slug_pt || defaults.slug_pt} / ${item.slug_en || defaults.slug_en}`,
      fields: publicationPageFields,
      blank: {
        id: pageId,
        slug_pt: defaults.slug_pt,
  ...
  async function loadCollection(preferLabel = "") {
    ...
    if (config.mode === "page-singleton") {
      ...
      let index = currentData[config.root].findIndex((item) =>
        item.id === config.pageId ||
      ...
      if (index < 0) {
        ...
        currentData[config.root][index] = {
          ...config.blank,
          ...currentData[config.root][index],
          id: config.pageId,
          slug_pt: currentData[config.root][index].slug_pt || config.blank.slug_pt,
          slug_en: currentData[config.root][index].slug_en || config.blank.slug_en
        };
      }
  ...
  async function saveCurrent() {
    ...
    if (config.mode === "page-singleton") {
      // Garante que a descrição seja salva no registro que a página pública lê.
      mergedValues.id = config.pageId;
      mergedValues.slug_pt = mergedValues.slug_pt || config.blank.slug_pt;
      mergedValues.slug_en = mergedValues.slug_en || config.blank.slug_en;
      mergedValues.intro_pt = String(values.intro_pt ?? mergedValues.intro_pt ?? "");
      mergedValues.intro_en = String(values.intro_en ?? mergedValues.intro_en ?? "");
    }
