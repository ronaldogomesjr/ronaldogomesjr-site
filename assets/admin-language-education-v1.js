(function () {
  ...
  const BRANCH = 'main';
  const PATH = 'content/educacao-linguistica.json';
  ...
  async function request(options = {}) {
    ...
    if (!accessToken) throw new Error('Insira e salve o token do GitHub antes de editar a página Educação linguística.');
  ...
  function setMessage(message, error = false) {
    const element = document.getElementById('languageEditorStatus');
    if (!element) return;
  ...
  function render() {
    const form = document.getElementById('languageEditorForm');
    if (!form || !data) return;
    form.innerHTML = '';
  ...
  async function load() {
    setMessage('Carregando a página Educação linguística…');
    try {
      const payload = await request();
      sha = payload.sha;
      data = JSON.parse(decodeBase64(payload.content));
      render();
      setMessage('Página Educação linguística carregada. Todos os textos abaixo podem ser editados.');
    } catch (error) {
  ...
  async function save() {
    ...
    try {
      ...
      const payload = await request({
        method: 'PUT',
        body: JSON.stringify({
          message: 'Atualiza conteúdo da página Educação linguística',
  ...
  function updateVisibility() {
    ...
    const panel = document.getElementById('languageCustomEditor');
    const grid = document.querySelector('.admin-grid');
    const addButton = document.getElementById('addBtn');
    const value = select?.value || '';
    const active = value === 'page-educacao-linguistica';
    const customActive = ['page-design', 'page-tecnologia-digital', 'page-educacao-linguistica'].includes(value);
  ...
  function initialize() {
    ...
    style.textContent = `.language-admin-panel[hidden] { display: none !important; }`;
    ...
    panel.id = 'languageCustomEditor';
    panel.className = 'admin-card language-admin-panel';
    panel.hidden = true;
    panel.innerHTML = `
      <h2>editar página Educação linguística</h2>
      <p class="hint">Título, descrição, perspectiva e SEO. As versões em português e inglês são salvas juntas.</p>
      <p id="languageEditorStatus" class="status">Clique em carregar para abrir os campos.</p>
      <form id="languageEditorForm" class="editor-form design-admin-form"></form>
      <div class="actions"><button id="languageEditorSave" type="button">publicar alteração</button></div>
    `;
    chooser.insertAdjacentElement('afterend', panel);

    select.addEventListener('change', updateVisibility);
    loadButton.addEventListener('click', (event) => {
      if (select.value !== 'page-educacao-linguistica') return;
      event.preventDefault();
      event.stopImmediatePropagation();
      load();
    }, true);

    document.getElementById('languageEditorSave').addEventListener('click', save);
    updateVisibility();
  }
